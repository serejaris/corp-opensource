...
|---|---|---|---|
| [aaif-goose/goose#9447](https://github.com/aaif-goose/goose/pull/9447) | Codex/OpenAI Responses parser: `response.completed` может приходить без `output`; добавлен regression-first parser test. | Open, review required; `check-quarantined` и `machete` passed; `changes` failed из-за GitHub diff API transient, dependent jobs skipped; mergeability сейчас `UNKNOWN`. | Ждать maintainer/rerun/review; кодовых действий нет. |
| [OpenHands/software-agent-sdk#3394](https://github.com/OpenHands/software-agent-sdk/pull/3394) | Уменьшен tmux viewport `1000x1000 -> 256x200`, добавлен regression на bounded viewport; review feedback закрыт follow-up commit `3c8eae78`. | Open, mergeable; GitHub всё ещё показывает старый `CHANGES_REQUESTED`; eval был triggered; workflows/check rollup пустой, fork branch ждёт maintainer/eval action. | Ждать maintainer eval / workflow approval. |
| [anomalyco/opencode#29565](https://github.com/anomalyco/opencode/pull/29565) | Structured output + thinking models: loosen tool-choice handling. | Open, mergeable; `check-duplicates`, `check-standards`, `check-compliance` passed. | Ждать maintainer review; не открывать третий opencode PR без отдельного сигнала. |
...
| [anomalyco/opencode#29530](https://github.com/anomalyco/opencode/pull/29530) | Webfetch: `format=null` трактуется как default, а не как schema error. | Open; mergeability сейчас `UNKNOWN`; duplicate/standards/compliance checks passed; contribution bot says guidelines satisfied. | Ждать maintainer review. |
| [CopilotKit/CopilotKit#5035](https://github.com/CopilotKit/CopilotKit/pull/5035) | Release bump не должен публиковать workspace deps. | Open, review required; docs Vercel preview green; other Vercel previews require team authorization; mergeability сейчас `UNKNOWN`. | Ждать maintainer review; Vercel auth failures не считать code failures. |
| [pydantic/pydantic-ai#5680](https://github.com/pydantic/pydantic-ai/pull/5680) | Сохранение `TextContent` metadata в UI adapter round-trip. | Open, mergeable; CI/harness/coverage green. | Ждать maintainer/bot review. |
...
| [modelcontextprotocol/typescript-sdk#2115](https://github.com/modelcontextprotocol/typescript-sdk/issues/2115) | `requestId: 0` cancel notification должен abort-ить matching request; regression card: [cycle 21](watch/ai-native-frameworks-scouting-2026-05-27-cycle-21.md), tracker [#51](https://github.com/serejaris/corp-opensource/issues/51). | COMMENT-FIRST / candidate: duplicate gate clean; [test-plan comment](https://github.com/modelcontextprotocol/typescript-sdk/issues/2115#issuecomment-4554920638) posted; wait maintainer signal before PR. |
...
| [langchain-ai/deepagents#3616](https://github.com/langchain-ai/deepagents/pull/3616) | Closed, not merged. | Assignment/maintainer gate; подход перенесён в issue-коммент. | Ждать maintainer assignment/reopen. |
...
| modelcontextprotocol/typescript-sdk | [#2115](https://github.com/modelcontextprotocol/typescript-sdk/issues/2115) | [#51](https://github.com/serejaris/corp-opensource/issues/51), [cycle 21](watch/ai-native-frameworks-scouting-2026-05-27-cycle-21.md) | COMMENT-FIRST / candidate; no assignee or duplicate PR found; upstream test-plan comment posted | Wait maintainer signal/assignment before PR |

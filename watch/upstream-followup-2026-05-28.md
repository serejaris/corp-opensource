...
Detailed note: [Codex/LangChain/opencode post-10:32 watch](codex-langchain-opencode-post1032-watch-2026-05-28-1042.md).

Process: parent live gates over post-10:32 signals, 6 read-only fallback roles, focused duplicate/PR checks, and process/actionability/duplicate critique. Required `open-source-bug-scouting` / `open-source-pr-workflow` skills are still unavailable here, so this used the documented fallback.
...
Tracker comment: umbrella [#52](https://github.com/serejaris/corp-opensource/issues/52#issuecomment-4563256336).

Material deltas: `codex#24896` is a weak usage/context drain `LEAD/WATCH` in an existing usage cluster (`#24337/#23136/#14593`), not a candidate without independent repro. `claude-code#63082` is duplicate-bot-linked to `#56172` but reporter disputes exact duplicate; keep product-owned watch. `opencode#29723` is duplicate/compliance/assignment-gated feature no-go; `opencode#29724` is support-refund no-go. `langchain#37732` is technically plausible but auto-closed as programmatic submission, so no upstream action.

Carry-forward: `probe#568`, `claude-peers-mcp#64`, `vercel/ai#15652`, and `gemini-cli#27503` remain `CANDIDATE-needs-runner-validation`; `Agent-S#195` remains the cheapest practical fallback after Probe/Claude-peers if runner tooling favors Python/unit tests.

## Claude/Codex/opencode post-10:26 watch, 2026-05-28 10:32 UTC

Detailed note: [Claude/Codex/opencode post-10:26 watch](claude-codex-opencode-post1026-watch-2026-05-28-1032.md).

Process: parent live gates over post-10:26 signals, 6 read-only fallback roles, focused duplicate/PR checks, and process/actionability/duplicate critique. Required `open-source-bug-scouting` / `open-source-pr-workflow` skills are still unavailable here, so this used the documented fallback.
...
Tracker comment: umbrella [#52](https://github.com/serejaris/corp-opensource/issues/52#issuecomment-4563197735).

Material deltas: `codex#24890` closed as user/config-side because `--oauth-resource` already covers that reporter path; keep broader `codex#13891` as open duplicate-heavy OAuth resource watch. `claude-code#63081` is duplicate-risk Cowork plugin validation UX, `#63082` is product-owned Desktop session data-loss watch, and `#63074` gained OTel `session.id` detail but remains duplicate-adjacent. `opencode#29721/#29722` is no-go because it is exact-covered, duplicate-heavy, assigned, and compliance-gated.

Carry-forward: `probe#568`, `claude-peers-mcp#64`, `vercel/ai#15652`, and `gemini-cli#27503` remain `CANDIDATE-needs-runner-validation`; `opencode#26187` remains conditional runner/source-test candidate but assignment-gated.

## opencode/Gemini/Serena runner watch, 2026-05-28 10:26 UTC

Detailed note: [opencode/Gemini/Serena runner watch](opencode-gemini-serena-runner-watch-2026-05-28-1026.md).

Process: parent live gates over post-10:18 signals, 6 read-only fallback roles, duplicate/PR checks where GitHub rate limits allowed, and process/actionability/duplicate critique. Required `open-source-bug-scouting` / `open-source-pr-workflow` skills are still unavailable here, so this used the documented fallback.
...
Tracker comments: umbrella [#52](https://github.com/serejaris/corp-opensource/issues/52#issuecomment-4563119534), Serena [#84](https://github.com/serejaris/corp-opensource/issues/84#issuecomment-4563119552).

Material deltas: `opencode#26187` is added as a runner/source-test candidate for Electron Desktop `webfetch format="text"` crashing on Bun-only `HTMLRewriter`; `serena#1519` is removed from active runner backlog because maintainer closed it fixed in `6dd3301e48`; `gemini-cli#27516` stays `WATCH / duplicate-covered` by active EBADF PRs `#27429/#27371`; `vercel/ai#15664/#15665` remains author-owned exact PR cover.

Carry-forward: first runner target remains `probe#568`; compact fallback remains `claude-peers-mcp#64`; high-impact candidates `vercel/ai#15652` and `gemini-cli#27503` remain blocked by dedicated runner evidence and repeat duplicate gates.
...
Detailed note: [opencode/LangChain permission-security watch](opencode-langchain-permission-security-watch-2026-05-28-1012.md).

Process: parent live gates over post-10:10 signals, reused 6 read-only fallback roles, and focused duplicate/PR checks. Required `open-source-bug-scouting` / `open-source-pr-workflow` skills are still unavailable here, so this used the documented fallback.
...
Tracker comment: umbrella [#52](https://github.com/serejaris/corp-opensource/issues/52#issuecomment-4562994890).

Material deltas: `opencode#29717` is an active external cover for `#29674` read/glob/grep permission enforcement and `**/.env*` root matching; `opencode#29718` is an active external cover for `#16491` subagent MCP permission inheritance. Both PRs are open/non-draft but `BLOCKED` by process/compliance. `langchain#37296` got fresh author-owned assignment/attribution pings around closed/unmerged PRs `#37291/#37729`; this stays `WATCH / author-owned process gate`, not our lane. `claude-code#63076` is duplicate-feature no-go.

Carry-forward: `claude-peers-mcp#64` remains the best compact source-level `CANDIDATE-needs-runner-validation`; `vercel/ai#15652`, `probe#568`, and `gemini-cli#27503` remain runner-backed candidates; `langchain#37728` remains duplicate-covered by `langchain-google#1704/#1708`.

## Claude Cowork/OTel/image watch, 2026-05-28 10:10 UTC

Detailed note: [Claude Cowork/OTel/image watch](claude-cowork-otel-image-watch-2026-05-28-1010.md).

Process: parent live gates over post-10:02 signals, reused 6 read-only fallback roles, and focused duplicate/PR checks. Required `open-source-bug-scouting` / `open-source-pr-workflow` skills are still unavailable here, so this used the documented fallback.
...
Tracker comment: umbrella [#52](https://github.com/serejaris/corp-opensource/issues/52#issuecomment-4562948899).

Material deltas: `claude-code#63075` is a fresh Windows Desktop/Cowork crash watch in a broad Cowork VM/crash cluster; `claude-code#63074` is OTel log correlation but strong-duplicate-adjacent to open `#55269`; `claude-code#63071` is Windows image paste regression with multiple adjacent open/closed image-paste issues; `claude-code#63072/#63073` are duplicate-labeled; `opencode#29716` self-closed as already existing. Active source-level candidates had no transition.
